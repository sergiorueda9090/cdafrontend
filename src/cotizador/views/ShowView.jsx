...
import { getAllThunks }               from '../../store/logsCotizadorStore/logsCotizadorThunks';
...
export const ShowView = () => {
  ...
  const returnCotizador = async() => {
    navigate(`/cotizador`);
  };

  const params = useParams();

  const { logsCotizador } = useSelector(state => state.logsCotizadorStore);
  ...
  return (
    <Box sx={{ height: 500, width: "100%", p: 3 }}>
     ...
     <Typography 
          ...
          onClick={returnCotizador} // Asegúrate de tener el evento onClick
        >
