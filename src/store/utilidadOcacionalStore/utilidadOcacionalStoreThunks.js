...
import { showStore, listStore, resetFormularioStore, handleFormStore  } from "./utilidadOcacionalStore.js";

// Función asincrónica para obtener los Pokemons
const parametersURL = 'utilidadocacional/api/';
...
export const getAllThunks = () => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'GET',
            url: `${ URL}/${parametersURL}utilidadocacional/`,
            headers: {
        ...
        try {
            ...
            if(response.status === 200){
                ...
                if(data.length > 0){
                    ...
                    await dispatch(listStore({'utilidadOcacionales':data}))

                    await dispatch(hideBackDropStore());

                }else{

                    await dispatch(listStore({'utilidadOcacionales':[]}))
...
export const createThunks = (data) => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'POST',
            url: `${URL}/${parametersURL}utilidadocacional/crear/`,
            headers: {
                Authorization: `Bearer ${token}`,
...
export const showThunk= (id = "") => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'GET',
            url: `${ URL}/${parametersURL}utilidadocacional/${id}/`,
            headers: {
              Authorization: `Bearer ${token}`
            }
          };
          ...
          try {
            ...
            if(response.status == 200){
                ...
                await dispatch(showStore(
                                            {   
                                                id                  : response.data.id ?? '',
                                                id_tipo_gasto       : response.data.id_tipo_gasto ?? '',
...
export const updateThunks = (data) => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'PUT',
            url: `${URL}/${parametersURL}utilidadocacional/${data.id}/update/`,
            headers: {
                Authorization: `Bearer ${token}`,
                'content-type': 'multipart/form-data; boundary=---011000010111000001101001'
              },
...
export const deleteThunk = (idUser = "") => {
    ...
    return async (dispatch, getState) => {
        ...
        const options = {
            method: 'DELETE',
            url: `${ URL}/${parametersURL}utilidadocacional/${idUser}/delete`,
            headers: {
              Authorization: `Bearer ${token}`
            }
          };
...
export const getAllThunksFilter = (fechaInicio, fechaFin) => {
    ...
    return async (dispatch, getState) => {
        ...
        let url = `${ URL}/${parametersURL}utilidadocacional/get_cotizadores_filter_date`;
